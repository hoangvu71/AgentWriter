...
class SQLiteTableManager:
    """Manages SQLite database tables and schema"""
    
    def __init__(self, connection_manager: SQLiteConnectionManager):
        """Initialize table manager with connection manager"""
    ...
    def create_indexes(self):
        ...
        indexes = [
            ...
            "CREATE INDEX IF NOT EXISTS idx_authors_session_id ON authors(session_id)",
            "CREATE INDEX IF NOT EXISTS idx_authors_user_id ON authors(user_id)",
            "CREATE INDEX IF NOT EXISTS idx_plots_session_id ON plots(session_id)",
            ...
            "CREATE INDEX IF NOT EXISTS idx_content_ratings_content_type ON content_ratings(content_type)",
            
            # Lore indexes
            "CREATE INDEX IF NOT EXISTS idx_lore_documents_cluster_id ON lore_documents(cluster_id)",
            
            # Agent invocations indexes
            "CREATE INDEX IF NOT EXISTS idx_agent_invocations_agent_name ON agent_invocations(agent_name)",
            "CREATE INDEX IF NOT EXISTS idx_agent_invocations_user_id ON agent_invocations(user_id)",
        ]
        
        for index_query in indexes:
            self.connection_manager.execute_query(index_query)
